...
CAD::OpenSCAD - A module to generate OpenSCAD files that can be used to create 3D structures using perl
...
be generated and produce files that can fed into a 3D printer.  *** NOTE API changed, the class was called
SCAD, but is nw renamesd to openSCAD for consistency with the module's file name and the library it depends
on.
